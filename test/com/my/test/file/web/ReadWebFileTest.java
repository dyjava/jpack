...
public class ReadWebFileTest {
	...
	private ReadWebFile readwebfile ;
...
//		url = "http://www.16k.cn/book/showbooklist.aspx?page=2" ;
		...
		readwebfile = new ReadWebFile() ;
	}

	@Test
	public void readUrlContentTest() {
		try {
			String content = readwebfile.readUrlContent(url, code) ;
	...
	public void readUrlContent2Test(){
		try {
			String content = readwebfile.readUrlContent2(url, code) ;
			Assert.assertNotNull(content) ;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void readWebFileTest(){
		try {
			String content = readwebfile.readWebFile(url, code) ;
			Assert.assertNotNull(content) ;
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
